...
structural:
  ...
  - { kind: stagger, literal: "-0.075", rule: stagger/shape }
...
1. `.menu-toggle-label p` → `y: "-110%"`, `duration: 1`, `ease: "{{motion.ease.primary}}"` — the word "Menu" slides up out of its overflow-hidden wrapper and disappears.
2. `.container` → `y: "100svh"`, `duration: 1`, `ease: "{{motion.ease.primary}}"` — the whole page is pushed down one full viewport (the "push-over").
3. `.menu-overlay` → `clipPath: "polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)"`, `duration: 1`, `ease: "{{motion.ease.primary}}"` — the overlay's bottom edge sweeps from the top of the screen down to the bottom, revealing the dark menu in a top-anchored wipe that moves in lockstep with the page being pushed down.
4. `.menu-overlay-content` → `yPercent: 0`, `duration: 1`, `ease: "{{motion.ease.primary}}"` — the content rides up from its initial `translateY(-50%)` to its natural position (a parallax counter-movement against the wipe).
...
- `tl.to(copyLines, { y: "0%", duration: 2, ease: "{{motion.ease.primary}}", stagger: -0.075 }, -0.15)`
...
**CLOSE timeline** (click while open): set `isAnimating = true`, remove the `active` class from the hamburger, then one timeline with five tweens all starting together (first at default position, the rest at `"<"`), all `duration: 1`, `ease: "{{motion.ease.primary}}"`:
