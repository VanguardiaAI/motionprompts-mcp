...
1. **Panel sweep-up** — `gsap.to(".menu", { clipPath: "polygon(0% 100%, 100% 100%, 100% 0%, 0% 0%)", pointerEvents: "all", duration: {{motion.duration.slow}}, ease: defaultEase })`. The clip-path goes from the bottom-collapsed line to a **full rectangle**, so the dark panel wipes upward to fill the screen; pointer events are enabled.
2. **Hero slides up + fades** — `gsap.to(".hero", { top: "-50%", opacity: 0, duration: {{motion.duration.slow}}, ease: defaultEase })`. The background photo pushes up off the top and fades to 0 as the panel covers it.
...
5. **Footer lines rise (staggered)** — `gsap.to(".menu-sub-item p", { y: 0, duration: {{motion.duration.base}}, stagger: 0.05, delay: 1, ease: "power3.out" })`. All footer mono lines slide from `y:12` up, a tighter 0.05s apart.
6. **Image stack slides in (staggered)** — `gsap.to(["#img-2, #img-3, #img-4"], { top: "50%", duration: {{motion.duration.slow}}, ease: defaultEase, stagger: 0.1, delay: 0.25, onComplete: () => { gsap.set(".hero", { top: "50%" }); isOpen = !isOpen; } })`. Images 2–4 rise from `top:150%` to the centered `top:50%`, 0.1s apart, layering behind `#img-1`. On complete it parks the (already invisible) hero at `top:50%` and flips `isOpen` true.
...
1. **Panel collapse off top** — `gsap.to(".menu", { clipPath: "polygon(0% 0%, 100% 0%, 100% 0%, 0% 0%)", pointerEvents: "none", duration: {{motion.duration.slow}}, ease: defaultEase })`. The clip-path collapses to a zero-height line at the **top edge**, so the dark panel wipes up and off; pointer events disabled.
2. **Menu column exits up + fades** — `gsap.to(".menu-items", { top: "-300px", opacity: 0, duration: {{motion.duration.slow}}, ease: defaultEase })`. The whole right column slides up 300px and fades.
3. **Hero slides back in + fades in** — `gsap.to(".hero", { top: "0%", opacity: 1, duration: {{motion.duration.slow}}, ease: defaultEase, onComplete: () => { /* full reset */ } })`. The hero animates from `top:50%` (parked below) back up to `top:0` while fading from 0→1, re-revealing the landing photo from below. Its `onComplete` **hard-resets everything** for the next open:
   ```js
