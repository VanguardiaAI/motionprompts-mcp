...
- `animateContentIn(titleChars, description)`:
  - chars (`.char span` NodeList) → `gsap.to(chars, { x: "0%", duration: {{motion.duration.slow}}, ease: "power4.out" })`
  - description → `gsap.to(desc, { x: 0, opacity: 1, duration: {{motion.duration.slow}}, delay: 0.1, ease: "power4.out" })`
