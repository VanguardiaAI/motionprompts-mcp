...
structural:
  ...
  - { kind: duration, literal: "0.2", rule: value/narrated }
  - { kind: stagger, literal: "0.03", rule: value/narrated }
...
1. **Block wipe in:** `tl.to(blocks, { scaleX: 1, duration: 0.4, stagger: {{motion.stagger.tight}}, ease: "power2.out", transformOrigin: "left" })` — each column grows from its **left** edge, staggering left→right across the screen (total ≈ 0.4 + 19×0.02 ≈ 0.78s) until the page is fully covered by the `#222` wall.
...
6. **Logo overlay fades off:** `.to(logoOverlay, { opacity: 0, duration: {{motion.duration.fast}}, ease: "power2.out" })` — revealing the solid block wall still standing beneath it.
...
- `gsap.to(blocks, { scaleX: 0, duration: 0.4, stagger: {{motion.stagger.tight}}, ease: "power2.out", transformOrigin: "right" })` — each column collapses toward its **right** edge, staggering across and uncovering the new page. `onComplete`: `isTransitioning = false` and `pointer-events:none` on both overlays.
...
gsap.to(split.chars, {
  y: "0%", duration: 1, stagger: 0.03, ease: "{{motion.ease.secondary}}", delay,   // delay = 0.3
