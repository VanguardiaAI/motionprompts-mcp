...
structural:
  - { kind: duration, literal: "18", rule: value/narrated }
...
```js
gsap.timeline({ defaults: { duration: {{motion.duration.base}}, ease: "{{motion.ease.primary}}" } })
...
loop?.kill();
gsap.set(strip, { x: 0 });
loop = gsap.to(strip, { x: -partWidth, duration: 18, ease: "{{motion.ease.primary}}", repeat: -1 });
```
