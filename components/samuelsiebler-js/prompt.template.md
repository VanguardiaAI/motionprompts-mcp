...
2. **Cycle detection / headline swap:** `newCycle = Math.floor((totalRotation + 90) / 360)`. When `newCycle !== currentCycle` and `0 <= newCycle < 5`, store it and call `updateHeaderText()`. Additionally:
   - When entering cycle 3 (and `!imageRevealed`): `gsap.to(handImage, { opacity: 1, duration: {{motion.duration.fast}} })` and `gsap.to(introCopy, { x: 0, opacity: 1, duration: {{motion.duration.fast}}, stagger: {{motion.stagger.base}} })` (the two paragraphs slide from x:20 to 0). Set `imageRevealed = true`.
   - When leaving cycle 3 (any other cycle while `imageRevealed`): reverse it — `gsap.to(handImage, { opacity: 0, duration: {{motion.duration.fast}} })`, `gsap.to(introCopy, { x: 20, opacity: 0, duration: {{motion.duration.fast}}, stagger: {{motion.stagger.base}} })`, `imageRevealed = false`.
