...
structural:
  ...
  - { kind: stagger, literal: "0.2", rule: value/narrated }
  - { kind: stagger, literal: "0.75", rule: value/narrated }
...
Register `CustomEase` and create a custom ease named `"hop"` from the cubic-bezier `0.9, 0, 0.1, 1`. On `DOMContentLoaded`, build ONE `gsap.timeline({ delay: 0.3, defaults: { ease: "{{motion.ease.primary}}" } })`. All tweens below use that ease unless stated.

1. **Counter roll.** Select all `.count` groups; for each `count` at `index`, grab its two `.digit h1`:
   - `tl.to(digits, { y: "0%", duration: 1, stagger: {{motion.stagger.tight}} }, index * 1)` — absolute position param: pair *n* starts exactly at *n* seconds into the timeline.
   - `tl.to(digits, { y: "-100%", duration: 1, stagger: {{motion.stagger.tight}} }, index * 1 + 1)` — each pair rolls up and out one second after it rolled in. Apply this to every pair (including the last). Net effect: each number slides up into the mask, holds implicitly via the overlap, and exits upward as the next slides in — a continuous odometer roll through 00, 27, 65, 98, 99 over ~5s.
...
6. **Block wipe + hero de-zoom.** `tl.to(".block", { clipPath: "polygon(0% 0%, 100% 0%, 100% 0%, 0% 0%)", duration: 1, stagger: 0.1, delay: 0.75 }, "<")` — both dark panels collapse to a zero-height strip at the top (an upward wipe), the second panel trailing the first by 0.1s. Its `onStart` fires `gsap.to(".hero-img", { scale: 1, duration: 2, ease: "{{motion.ease.primary}}" })` so the background image settles from 1.5 → 1 while the blocks lift.
