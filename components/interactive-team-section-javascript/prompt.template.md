...
4. **Per-thumbnail hover (index i pairs with name i+1, skipping the default):**
   - `mouseenter` on `.img`:
     ...
     - Tween that member's letters: `y: "-100%", duration: {{motion.duration.base}}, ease: "power4.out", stagger: { each: 0.025, from: "center" }` → the name rises into the mask, characters unfolding from the middle outward.
   - `mouseleave` on `.img`:
     - Tween the wrapper back: `width: 70, height: 70, duration: 0.5, ease: "power4.out"`.
     - Tween the letters back: `y: "0%", duration: {{motion.duration.base}}, ease: "power4.out", stagger: { each: 0.025, from: "center" }` → the name sinks back below the mask.
5. **Row-level hover (default title):**
   - `mouseenter` on `.profile-images` container: tween the default letters to `y: "0%", duration: {{motion.duration.base}}, ease: "power4.out", stagger: { each: 0.025, from: "center" }` → "The Squad" slides **up and out** of view (so a member name can take the stage).
