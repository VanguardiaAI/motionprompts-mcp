...
structural:
  - { kind: duration, literal: "5", rule: value/narrated }
...
1. **Images rise + fade in:** `images` → `{ y: 0, opacity: 1, stagger: {{motion.stagger.tight}}, duration: 1 }`. (From `translateY(50%) scale(0.5) opacity:0` to `y:0 opacity:1`, still at scale 0.5, left-to-right stagger.)
...
4. **Side images wipe away upward:** `images.filter((_, i) => i !== heroIndex)` (the 4 non-center images) → `{ clipPath: "polygon(0% 0%, 100% 0%, 100% 0%, 0% 0%)", duration: 1, stagger: {{motion.stagger.base}} }`. The clip-path collapses each image's bottom edge up to the top edge, so they vanish upward one after another, leaving only the center image.
...
7. **Hero name slides in word by word** (position param `"-=0.5"`, i.e. starts 0.5s before step 6 ends, overlapping the overlay wipe): `".hero-header h1 .word"` → `{ y: "0", duration: 0.75, stagger: {{motion.stagger.base}}, ease: "power3.out" }`. Each word slides up from `translateY(100%)` into its mask (`ease: "power3.out"`, NOT hop, for this one).
