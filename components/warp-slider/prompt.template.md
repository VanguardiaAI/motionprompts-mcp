...
structural:
  ...
  - { kind: duration, literal: "0.6", rule: value/narrated }
...
- `gsap.fromTo(chars, { y: "100%" }, { y: "0%", duration: {{motion.duration.base}}, stagger: {{motion.stagger.tight}}, ease: "power2.out" })`
- `gsap.fromTo(lines, { y: "100%" }, { y: "0%", duration: {{motion.duration.base}}, stagger: {{motion.stagger.tight}}, ease: "power2.out", delay: 0.2 })`
...
- **Chars out:** `.to(currentContent.querySelectorAll(".char span"), { y: "-100%", duration: 0.6, stagger: {{motion.stagger.tight}}, ease: "power2.inOut" })` at time 0.
...
- **`.call(fn, null, 0.5)`** — at the 0.5s mark (while the outro tails are still finishing):
  ...
  4. **`setTimeout(…, 100)`** (lets layout settle so SplitText measures real line breaks), then:
     ...
     - a new timeline with `onComplete: () => { isTransitioning = false; currentSlideIndex = nextIndex; }`:
       - **Chars in:** `.to(newChars, { y: "0%", duration: 0.5, stagger: {{motion.stagger.tight}}, ease: "power2.inOut" })` at time 0.
