...
tl.to(element, {
  ...
  duration: {{motion.duration.base}},                     // CONFIG.animationDuration
  ease: "{{motion.ease.exit}}",                  // accelerate away
...
tl.to(element, {
  ...
  duration: {{motion.duration.base}},                         // CONFIG.animationDuration
  ease: "{{motion.ease.primary}}",                     // decelerate into place
...
```js
gsap.timeline()
  .to(galleryHeading, { opacity: 0, duration: 0.5, ease: "{{motion.ease.primary}}" })
  .call(() => { galleryHeading.textContent = newText; })
  .to(galleryHeading, { opacity: 1, duration: 0.5, ease: "{{motion.ease.primary}}" });
```
