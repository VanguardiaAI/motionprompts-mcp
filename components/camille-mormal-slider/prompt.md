...
The curve rises steeply and reaches ~1 early (around x≈0.5) then flattens — a fast "snap-then-settle". **Every** tween in this component uses `ease: "hop"`.
...
function place(duration) {
  odometers.forEach((track) => {
    gsap.to(track, {
      ...
      duration, ease: "hop", overwrite: true,
    });
  });
}
```
...
gsap.to(railFill, { scaleX: current / total, duration: STEP, ease: "hop" });
...
4. **Push the outgoing image off**: `gsap.to(outgoing.querySelector("img"), { x: direction === "left" ? push : -push, duration: SLIDE, ease: "hop" })`.
5. **Wipe the incoming slide open** from the edge you came from — with the horizontal edges of the polygon pushed 2% *outside* the plate:
   ...
   gsap.fromTo(frame,
     ...
     { clipPath: "polygon(0% -2%, 100% -2%, 100% 102%, 0% 102%)",
       duration: SLIDE, ease: "hop", onComplete: dropSpentFrames });
   ```
...
6. **Parallax the incoming image to rest**: `gsap.to(img, { x: 0, duration: SLIDE, ease: "hop" })`. The picture drifts into place *while* the mask opens over it — the two move at different rates, which is the parallax.
...
8. **Rotate the marks**: `markRotation += direction === "left" ? -90 : 90; gsap.to(marks, { rotate: markRotation, duration: STEP, ease: "hop" })`. A `+` at 90° reads as `×`, so the two marks flip between `+` and `×` on every step and keep winding.
