...
structural:
  ...
  - { kind: stagger, literal: "0.01", rule: value/narrated }
  ...
  - { kind: ease, literal: "\"elastic.out(1, 0.25)\"", rule: ease/overshoot }
  - { kind: ease, literal: "\"power2.out\"", rule: value/narrated }
  - { kind: ease, literal: "\"power3.in\"", rule: value/narrated }
...
### 3. openMenu()
Add `is-open` to `.menu`. First, cross-fade the toggle words (independent tweens, `ease: "{{motion.ease.primary}}"`):
- `navToggleMenu` → `{ opacity: 0, duration: {{motion.duration.fast}} }` ("Menu" out).
- `navToggleClose` → `{ opacity: 1, duration: {{motion.duration.fast}}, delay: 0.25 }` ("Close" in, after the other fades).
...
3. **Menu logo** at position **`"-=0.75"`** (i.e. absolute t≈0.25) — `menuLogo` → `{ opacity: 1, duration: {{motion.duration.fast}}, ease: "{{motion.ease.primary}}" }`.
4. **Contact info** at position **`"-=0.35"`** (i.e. absolute t≈0.65) — `menuInfoItems` → `{ opacity: 1, y: 0, duration: 0.75, ease: "power3.out", stagger: {{motion.stagger.base}} }` (label, then each line, rise up from `y:100`).
...
### 4. closeMenu()
First `gsap.set(menuBg, { attr:{ d: CLOSE_START } })` — instantly re-anchor the (currently full) curtain to the bottom so its moving edge becomes the flat top. Cross-fade the toggle words back (`ease: "{{motion.ease.primary}}"`):
- `navToggleClose` → `{ opacity: 0, duration: {{motion.duration.fast}} }`.
- `navToggleMenu` → `{ opacity: 1, duration: {{motion.duration.fast}}, delay: 0.25 }`.
...
Timeline tweens:
1. `menuLogo` → `{ opacity: 0, duration: {{motion.duration.fast}} }`.
2. `menuLinks` (the `<a>` elements, not the chars) → `{ opacity: 0, duration: {{motion.duration.fast}} }` at `"<"` (same time as #1).
3. `menuInfoItems` → `{ opacity: 0, duration: {{motion.duration.fast}} }` at `"<"`.
