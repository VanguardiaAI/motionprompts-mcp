...
structural:
  ...
  - { kind: stagger, literal: "0.5", rule: value/narrated }
...
2. `tl.to(".img", { scale: 1, duration: 1, stagger: {{motion.stagger.base}}, ease: "power3.out" }, "<")` — at
...
- `.to(img, { scale: 2.5, duration: {{motion.duration.fast}}, ease: "power3.in" }, 0.025)` — grows while leaving
  the corner,
- `.to(img, { scale: 1, duration: {{motion.duration.fast}}, ease: "power3.out" }, 0.5)` — shrinks back as it lands
  bottom-right.
