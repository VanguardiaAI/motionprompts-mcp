...
```
https://motionprompts.dev/c/larevoltosa-scroll-animation/img.jpg
```
...
- **Palette on `:root`** — `--coral`, `--aubergine`, `--cream`, `--purple`, `--blue`. These names are not namespaced and they collide: `--ink` is defined by 164 of the 219 components in this catalogue, `--paper` by 94, `--muted` by 80, each with different values — and they will also collide with whatever your own project defines. Move them onto the component's wrapper (`.my-section { --ink: … }`) or rename them with a prefix.
- **Rules on `*`, `body`** — the demo owns the whole document, so these set the page background, typography and resets. Dropped into an existing project they restyle the entire page, not just this section. Re-target them at the component's wrapper before using it.
- **Smooth scroll (Lenis)** — this creates its own Lenis instance, and a page may only have one. If your project already runs Lenis, drop the setup shown above and reuse the existing instance, keeping the `lenis.on("scroll", ScrollTrigger.update)` wiring once. Two instances fight over the same scroll and stutter visibly, with no error in the console.

## Adapting this to React

Everything above describes a standalone document: a script that waits for `DOMContentLoaded`, injects the doodle's raw SVG markup into `.svg-container`, tears two of its paths apart at runtime to build the lens `clipPath`s, and only then wires the single pinned `ScrollTrigger` that drives the glare sweep, the 1→43 zoom, the lens photo fade and the word-by-word headline from one `onUpdate`. React withdraws the ready-made document, the free run of unscoped selectors and the license to never tear anything down — and it does it quietly: the first mount pins `.spotlight`, zooms into the glasses and looks correct, and the damage only shows up on a second mount or a real navigation away.

Under React 19 with StrictMode, every effect mounts, unmounts, and mounts again before anything reaches the screen. By the time the first mount's `DOMContentLoaded` body has run, it has live state hanging off of it: a `Lenis` instance, a `gsap.ticker` callback pumping it every frame, a pinned, scrub-driven `ScrollTrigger` whose `pinSpacing` has already reserved three viewport-heights of extra scroll under `.outro`, a `headerSplit` that has rewritten `.spotlight-header h1`'s children into `.spotlight-word` spans, and the freshly-built `lensImages`/`glareBands` arrays closed over by that trigger's `onUpdate`. A second, un-cleaned-up run of the same body does not just duplicate work — `.svg-container`'s `innerHTML` gets reassigned, so the first pass's `<svg>`, its clip groups, its lens `<image>`s and glare `<rect>`s are all detached from the document, but the *first* `ScrollTrigger` is still alive, still pinned, and its `onUpdate` keeps calling `gsap.set` against those now-orphaned nodes on every scroll frame. Meanwhile the second pass's `ScrollTrigger.create({ trigger: ".spotlight", pin: true, pinSpacing: true, ... })` pins the same section a second time, so the page now reserves six viewport-heights of scroll instead of three, with two pin-spacers stacked and two zoom progressions disagreeing about where you are. And because `.spotlight-header h1` is a real, static element that survives the remount untouched, the second `SplitText.create(spotlightHeader, { type: "words", wordsClass: "spotlight-word" })` runs on a node that is already full of `.spotlight-word` spans from the first split — it splits the split, and `headerSplit.words` in the live trigger no longer lines up one-to-one with the words a reader actually sees.

*(1) The entry point* — delete the `DOMContentLoaded` listener and move its entire body — the Lenis/ticker wiring, the `settings` object, the `.svg-container` injection and lens-building loop, the `SplitText.create` call, the `gsap.set` that primes `transformOrigin`/`transformBox` on the doodle, and the `ScrollTrigger.create` — into a `useEffect` with an empty dependency array. `gsap.registerPlugin(ScrollTrigger, SplitText)` can stay at module scope; re-registering per mount is harmless but pointless.

*(2) Element lookups* — this component's riskiest lookup is not the class selectors, it's `const svg = document.querySelector("svg")` right after the `innerHTML` injection: a bare tag selector that matches the *first* `<svg>` anywhere in the document, not necessarily the one this effect just wrote. Scope every lookup to a root ref instead: `root.querySelector(".svg-container")` to find the container, then `.querySelector("svg")` on *that* node — never on `document`. Do the same for `.spotlight-header h1` and `.spotlight` (the pin trigger). During the StrictMode remount, two copies of the injected doodle briefly coexist; an unscoped `document.querySelector("svg")` has no way to prefer the one this effect actually owns over the stale one still waiting to be torn down.

*(3) Cleanup* — wrap the whole body in a `gsap.context` scoped to the root ref, and use its teardown to unwind the pieces `ctx.revert()` does not reach on its own:
...
useEffect(() => {
  ...
  const ctx = gsap.context(() => {
    // svg injection, clip/lens/glare construction, SplitText.create,
    // the priming gsap.set, and ScrollTrigger.create — exactly as above,
    // all lookups scoped to `root` instead of `document`
  }, root);
...
`ctx.revert()` covers the `ScrollTrigger` (removing the pin-spacer it wrapped around `.spotlight` and restoring the section's own inline styles, so the extra reserved scroll height collapses back to zero) and the `gsap.set` calls the plugin tracked on the `<svg>`, the glare bands and the lens images. It does **not** cover `gsap.ticker.add(onTick)` — a ticker subscription is neither a tween nor a trigger — so that callback has to be removed by the same function reference you passed in, and it has to happen before `lenis.destroy()`: a ticker frame that lands between reverting the context and destroying Lenis would still call `.raf()` on a live instance, which is harmless, but one that lands *after* destroy and *before* `gsap.ticker.remove` calls `.raf()` on an instance with nothing left to drive.

The split needs the same explicit treatment: call `headerSplit.revert()` inside the same cleanup (or let it happen as part of the context if your GSAP version reverts context-created splits automatically — check before relying on it) before the next mount's `SplitText.create` runs against `.spotlight-header h1` again. Skipping this is exactly what produces the nested-span bug described above: the second split's `wordsClass: "spotlight-word"` wraps spans that are already wrapped, and `headerSplit.words.length` — the denominator the word-reveal math in `onUpdate` divides by — stops matching the number of words a reader can actually see.
