...
```
Exact values:
- **`Flip.from` duration `1.5`s, `ease:"hop"`.** Flip interpolates each tile's position AND size (100×125 ↔ 75×100 ↔ 300×400) between layouts — tiles slide, scale, and re-pile in one continuous move.
...
- Both are `gsap.to` with `ease:"none"`, `duration:0.1` — a short smoothing tween each scroll event, riding on top of Lenis's already-smoothed scroll.
