...
structural:
  ...
  - { kind: ease, literal: "\"glide\"", rule: value/narrated }
...
5. **Logo fades out.** `.to("#pbc-logo", { opacity: 0, duration: {{motion.duration.fast}}, ease: "power1.out" }, "-=0.25")` — overlaps back 0.25s into the last progress jump.
...
1. **Preloader scales down.** `.to(".preloader", { scale: 0.75, duration: {{motion.duration.base}}, ease: "hop" })`.
2. **Both strokes unwind off the ring.** `.to([btnOutlineTrack, btnOutlineProgress], { strokeDashoffset: -svgPathLength, duration: {{motion.duration.base}}, ease: "hop" }, "<")` — offset goes to **negative** circumference, so both circles animate out of view (unraveling), starting together with step 1.
...
7. **Hero scales up.** `.to(".hero", { scale: 1, duration: {{motion.duration.base}}, ease: "hop" })` — hero eases 0.75 → 1 (settles to full size as it's revealed).
