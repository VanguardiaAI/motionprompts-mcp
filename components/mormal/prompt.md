...
8. **Content reveals up through the clip-path masks.**
   ...
   tl.to(".nav-item a, .title p, .slide-num p, .preview img",
     { top: 0, stagger: 0.075, duration: 1, ease: "power3.out" }, "-=1.5");
   ```
...
9. **The two "+" icons pop in.**
   ...
   tl.to(".icon ion-icon, .icon-2 ion-icon",
     { scale: 1, stagger: 0.05, ease: "power3.out" }, "-=1");
   ```
   The two hero "+" glyphs scale `0` → `1` (**no `duration` set → GSAP default 0.5s**), `stagger: 0.05`, `ease: "power3.out"`. `"-=1"` → starts at **t ≈ 6.25s**.
